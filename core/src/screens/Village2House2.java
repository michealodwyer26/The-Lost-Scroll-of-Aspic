...
public class Village2House2 implements Screen {
	...
	public Village2House2(RpgGame game) {
		this.game = game;
		
		map = new TmxMapLoader().load("data/maps/village2_house2.tmx");
		...
		if(player.isWalkingToDestination) {
			if(player.getWalkingToDestinationTimer() > RpgGame.FAST_SCREEN_TRANSITION) {
				if(player.destination.equals("Village 2")) {
					game.setScreen(new Village2(game, 112f, 512f, "south"));
				}
			}
		}

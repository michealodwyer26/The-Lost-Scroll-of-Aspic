...
public class Village2House1 implements Screen{
	...
	private FemaleVillager2 femaleVillager2;
	...
	public Village2House1(RpgGame game) {
		this.game = game;
				
		map = new TmxMapLoader().load("data/maps/village2_house1.tmx");
		...
		femaleVillager2 = new FemaleVillager2(game, mapObjectsRects, player.rect);
		mapRenderer.addSprite(femaleVillager2);
	...
	public void render(float delta) {
		...
		femaleVillager2.update(delta);
		
		Gdx.gl.glClearColor(0, 0, 0, 0);
		Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
		
		if(!player.isWalkingToDestination) {
			player.checkCollisions(mapObjectsRects, specialMapObjectsRects, femaleVillager2.rect);
		}
		
		femaleVillager2.checkCollisions();
		
		if(player.isWalkingToDestination) {
			if(player.getWalkingToDestinationTimer() > RpgGame.FAST_SCREEN_TRANSITION) {
				if(player.destination.equals("Village 2")) {
					game.setScreen(new Village2(game, 80f, 112f, "south"));
				}
			}
		}
		...
		if(debugLines) {
			...
			shapeRenderer.rect(femaleVillager2.rect.x, femaleVillager2.rect.y, femaleVillager2.rect.width, femaleVillager2.rect.height);
			shapeRenderer.end();
		}

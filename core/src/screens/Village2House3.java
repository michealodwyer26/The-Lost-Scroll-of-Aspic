...
public class Village2House3 implements Screen {
	...
	private MaleVillager2 maleVillager2;
	...
	public Village2House3(RpgGame game) {
		this.game = game;
				
		map = new TmxMapLoader().load("data/maps/village2_house3.tmx");
		gamecam = new OrthographicCamera();
		gamePort = new FitViewport(RpgGame.V_WIDTH, RpgGame.V_HEIGHT, gamecam);
		
		player = new Player(game, 6 * RpgGame.TILE_SIZE, 0, "north");
		...
		maleVillager2 = new MaleVillager2(game, mapObjectsRects, player.rect);
		mapRenderer.addSprite(maleVillager2);
	...
	public void render(float delta) {
		...
		maleVillager2.update(delta);
		
		Gdx.gl.glClearColor(0, 0, 0, 0);
		Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
		
		if(!player.isWalkingToDestination) {
			player.checkCollisions(mapObjectsRects, specialMapObjectsRects, maleVillager2.rect);
		}
		
		if(player.isWalkingToDestination) {
			if(player.getWalkingToDestinationTimer() > RpgGame.FAST_SCREEN_TRANSITION) {
				if(player.destination.equals("Village 2")) {
					game.setScreen(new Village2(game, 448f, 144f, "south"));
				}
			}
		}
		
		maleVillager2.checkCollisions();
		...
		if(debugLines) {
			...
			shapeRenderer.rect(maleVillager2.rect.x, maleVillager2.rect.y, maleVillager2.rect.width, maleVillager2.rect.height);
			shapeRenderer.end();
		}

...
public class Village1House1 implements Screen {
	...
	private FemaleVillager femaleVillager;
	...
	public Village1House1(RpgGame game) {
		this.game = game;
		
		map = new TmxMapLoader().load("data/maps/house1.tmx");
		gamecam = new OrthographicCamera();
		gamePort = new FitViewport(RpgGame.V_WIDTH, RpgGame.V_HEIGHT, gamecam);
		
		player = new Player(game, 2 * RpgGame.TILE_SIZE, 0, "north");
		...
		femaleVillager = new FemaleVillager(game, mapObjectsRects, player.rect);
		mapRenderer.addSprite(femaleVillager);
	...
	public void handleInput() {
		...
		if(!displayMenu && !player.isWalkingToDestination) {
			...
			if(Gdx.input.isKeyPressed(Input.Keys.DOWN) && !isWalking) {
				...
				if(displayDialogueBox == true) {
					displayDialogueBox = false;
					
					if(player.getX() == 64f && player.getY() == 64f) {
						displayDialogueBox = true;
	
					}
					else if(player.getX() == 96f && player.getY() == 64f) {
						displayDialogueBox = true;
					}
				}
			}
				...
				if(displayDialogueBox == true) {
					...
					if(player.getX() == 80f && player.getY() == 0f) {
						displayDialogueBox = true;
					}
				}
				...
				if(displayDialogueBox == true) {
					...
					if(player.getX() == 16f && player.getY() == 64f) {
						displayDialogueBox = true;
					}
				}
			}
	...
	public void render(float delta) {
		...
		femaleVillager.update(delta);
		
		Gdx.gl.glClearColor(0, 0, 0, 0);
		Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
		
		if(!player.isWalkingToDestination) {
			player.checkCollisions(mapObjectsRects, specialMapObjectsRects, femaleVillager.rect);
		}
		
		femaleVillager.checkCollisions();
		
		if(player.isWalkingToDestination) {
			
			if(player.getWalkingToDestinationTimer() > RpgGame.FAST_SCREEN_TRANSITION) {
				if(player.destination.equals("Village")) {
					game.setScreen(new Village1(game, 160f, 80f, "south"));
				}
			}
		}
		...
		if(debugLines) {
			...
			shapeRenderer.rect(femaleVillager.rect.x, femaleVillager.rect.y, femaleVillager.rect.width, femaleVillager.rect.height);
			shapeRenderer.end();
		}
